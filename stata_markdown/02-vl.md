...
First, note that with User Variable Lists, the `vl copy` command **does not work**. 

We can create new lists which build off old lists with `vl create`. To add a new variable:
...
We can also merge or remove lists themselves.
...
<<dd_do>>
vl dir, user
vl drop mylist3 mylist4 mylist5
vl dir, user
<</dd_do>>
~~~~
